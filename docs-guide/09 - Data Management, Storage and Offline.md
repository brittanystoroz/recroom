...
We face an additional set of challenges maintaining our data when developing for mobile devices. Because users may be travelling in and out of online areas, it's important to have a good strategy around handling offline data.
...
Ember Data is not tied to IndexedDB by default, (it is agnostic to underlying technologies), so we have included the npm package [ember-indexeddb-adapter](http://#)] to help persist our data.
**code example**
